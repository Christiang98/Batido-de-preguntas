...
function Timer({ timerEnd }) {
  ...
  return (
    <div className="timer-display si">
      <div className={`timer-num ${urgent ? 'urgent' : 'normal'}`}>
        {mins}:{secs}
      </div>
  ...
  useEffect(() => {
    ...
    unsubs.push(onValue(ref(db, 'groups'), snap => {
      if (snap.exists()) {
        ...
        setMyGroup((g.list || []).find(gr => (gr.members || []).includes(name)) || null)
      } else {
    ...
    <>
      ...
      <div className="page">
        ...
        <div className="header fu">
          ...
          <button onClick={leave} style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: '13px', fontFamily: 'var(--body)' }}>
            Salir
          </button>
        </div>
        ...
        {!isActive && (
          <>
            <div className="fu fu1" style={{ marginBottom: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '0.75rem' }}>
                <div className="chip chip-green"><div className="live-dot" /> En vivo</div>
              </div>
              <div className="display-title" style={{ fontSize: '32px' }}>Sala de<br/>espera</div>
          ...
          <>
            ...
            {myGroup ? (
              <div className="card-my-group fu fu2">
                <div className="section-label" style={{ color: 'var(--gold2)', marginBottom: '0.85rem' }}>
                  ✦ Tu grupo
                </div>
              ...
              <div className="card fu fu2">
                <div className="empty">
                  <div className="empty-icon">🔍</div>
                  No encontramos tu grupo. Avisale al animador.
                </div>
              </div>
            )}
              ...
              <div className="fu fu3 mt1">
                ...
                {groups.map((g, i) => (
                  <div key={i} className="group-admin-card">
                    <div className="group-num">Grupo {i + 1}</div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                      {(g.members || []).map((m, j) => (
                        <div key={j} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', color: m === myName ? 'var(--gold2)' : 'var(--muted)' }}>
