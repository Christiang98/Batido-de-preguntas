...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    unsubs.push(onValue(ref(db, 'participants'), snap => {
      if (snap.exists()) {
        setParticipants(Object.entries(snap.val()).map(([id, v]) => ({ id, ...v })))
  ...
  async function generateGroups() {
    ...
    try {
      ...
      const { groups: list, newPairHistory } = makeGroupsWithRotation(names, groupSize, pairHistory)
      ...
      await set(ref(db, 'groups'), { list, dinamica: din, usedDinamicas: newUsed, pairHistory: newPairHistory, timerEnd })
      await set(ref(db, 'session'), { status: 'active', round })
      setTab('groups')
    } catch (err) {
      alert('Error al generar grupos.'); console.error(err)
    }
    setLoading(false)
  }
      ...
      <div className="page">
        ...
        <form onSubmit={login} className="fu fu1 btn-stack">
          <input
            ...
            autoFocus
          />
          {pwError && <div style={{ fontSize: '13px', color: 'var(--red2)', fontWeight: 500 }}>⚠ {pwError}</div>}
          ...
          <div className="fu">
              ...
              {participants.map(p => (
                <div key={p.id} className="person-row">
                  ...
                  <button
                    ...
                    style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: '20px', lineHeight: 1, padding: '0 4px' }}
                  >×</button>
                </div>
              ))}
            </div>
            ...
            <div className="card mb1">
              ...
              {Object.keys(pairHistory).length > 0 && (
                <div style={{ marginTop: '12px', padding: '10px 12px', background: 'rgba(0,229,160,0.07)', border: '0.5px solid rgba(0,229,160,0.2)', borderRadius: '8px' }}>
                  <div style={{ fontSize: '12px', color: 'var(--green)', fontWeight: 600 }}>
                    ✓ Rotación inteligente activa — los grupos evitan repetir compañeros
                  </div>
                </div>
              )}
            </div>
